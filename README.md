...
(When using a closed-source LLM like ChatGPT, we will directly prompt it for generation and utilize only the stage 2&3 to improve the quality of training corpus. Through our setups, we found that using only these two stages has been very helpful. So far, this repository mainly contains source code for stage 2&3, and we are still working on cleaning up the stage 1 code.)
